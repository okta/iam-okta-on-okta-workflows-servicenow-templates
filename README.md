...
* **Okta Tenant:** Access to an Okta tenant with Okta Workflows enabled for your organization.
* **ServiceNow Tenant:** Access to a ServiceNow tenant with the **Integration Hub** plugin installed.
* **Access Request ID:** An Application Access Request ID ([Okta Documentation](https://help.okta.com/oie/en-us/content/topics/identity-governance/access-requests/ar-get-started.htm)).
* **Workflow Template:** Download the [Automate SSO Application Creation Template](https://github.com/okta/workflows-templates/tree/master/workflows/automate_sso_application_creation).
...
1. **Download Source:** Download the `sys_remote_update_set_[sys_id].xml` file from the **ServiceNow Application** folder.
2. **Import to ServiceNow:** Navigate to **System Update Sets** > **Retrieved Update Sets** and select **Import Update Set from XML**.
3. **Preview & Resolve Errors:** Run a preview on the update set. Expect errors during this step—review them and select **Accept Remote Update** for all errors to allow the commit to proceed.
4. **Commit Update Set:** Commit the update set to apply the changes to your instance.
5. **Set Application Scope:** Ensure your system's application scope is set to `Okta Workflows`.
